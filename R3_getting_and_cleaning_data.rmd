...
We're going to work with Social Security Administration data on names of babies born across the last ~100 years. It comes built in with the 'babynames' package we loaded. 
...
```{r}
# run without having to log in to load this public sheet
gs4_deauth()
...
538 (RIP) put a lot of neat data sets on its' GitHub organization page.  Here's the page to a dataset about the Avengers, from Marvel Comics:  
...
* Get data from packages the R community has built to load and work with specific kinds of data, including some nice helper functions. If the dataset you want to use is popular, there's a great change someone has built an R package for it! One great example:
